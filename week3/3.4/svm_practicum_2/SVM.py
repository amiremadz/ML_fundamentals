...
class SVM(object):
    ...
    def gram_matrix(self, X):
        # Trying vectorized approach but getting error
        '''X=np.asarray(X)
        factor=np.diag(X[:,0])
        return self.kernel(factor,X)'''
        # Storing the row and column length of Matrix
        row, col = X.shape
        # Creating a matrix with zeros
        gramMat = np.zeros((row, row))
        # Creating the outer and inner loop
        # and using enumerate to store the index

        for i, label in enumerate(X):
            for j, feature in enumerate(X):
                gramMat[i, j] = self.kernel(label, feature)
        return gramMat
    ...
    def solve(self, X, y):
        ...
        # Storing the shape of matrix in m,n
    ...
    def _predict(self, X, bias, support_weights, support_vectors, support_vector_labels, kernel):
        """
        This is an internal method used in two different locations.
        It computes the SVM cost function sum, and thus
        provides prediction labels.
        """
        result = bias
        for suppWeight, suppVec, suppVecLabel in zip(support_weights,
                                 support_vectors,
                                 support_vector_labels):
            result += suppWeight * suppVecLabel * kernel(suppVec, X)
